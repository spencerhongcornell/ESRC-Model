...
def massabsorberevaporator(m6, m4, xa4, ya3, xa6):
	m3, m5= symbols([ 'm3', 'm5'])
	system = [
	Eq((xa4*m4)+ (ya3 * m3)  - (xa6 * m6), 0),
	Eq(m4 + m3 - m5 - m6, 0)
	]
	soln = solve(system, [m3, m5])

	return float(soln[m3]), float(soln[m5])
...
def Qgenerator(massin, compin):
	...
	enthalpyout = interpolate('enthalpy-kjmol-330K-ammoniawater', compin)
...
def Sgenerator(massin, compin, Qgen):
	...
	entropyout = interpolate('entropy-kjmol-330K-ammoniawater', compin)

	Sgen = symbols('Sgen')
	system = [
	Eq((-1 * massin * entropyin ) + (massout*entropyout) + (Qgen/330) - Sgen, 0)
...
def Qevaporator(m2, m3, m5, ya3, ya2, xa5):
	...
	enthalpym2 = interpolate('enthalpy-kjmol-330K-ammoniawater', ya2)
...
def Sevaporator(m2, m3, m5, ya3, ya2, xa5, T, Qevap):
	...
	entropym2 = interpolate('entropy-kjmol-330K-ammoniawater', ya2)
...
def Qabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6):
	...
	enthalpym4 = interpolate('enthalpy-kjmol-330K-ammoniawater', xa4)
...
def Sabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6, Qabs, T):
	...
	entropym4 = interpolate('entropy-kjmol-330K-ammoniawater', xa4)
...
compammoniagen = 0.30  # this is the composition of ammonia out of the generator
################
#    RAHUL     #
################
...
massvaporoutflash, massliquidoutflash, liquidammoniacomp, vaporammoniacomp = leverrule(massintoflash, 330, compammoniagen)
...
m3, m5 = massabsorberevaporator(m6, m4, xa4, ya3, xa6)
...
Qabs = Qabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6)
Sabs = Sabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6, Qabs, 325)

print("Qabs: " + str(Qabs) + "\nSabs: " + str(Sabs))

Qgen = Qgenerator(m6, xa6)
Sgen = Sgenerator(m6, xa6, 330)

print("Qgen: " + str(Qgen) + "\nSgen: " + str(Sgen))

print(Qevap/Qgen)
#Sall = Sgen + Sevap + Sabs
COPrev = reversibleCOP(266, 325, 330)

#Sall = - Qevap/266 - Qabs/325 + Qgen/330
