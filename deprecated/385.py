...
def Qgenerator(massin, compin):
    ...
    enthalpyout = interpolate('enthalpy-kjmol-385K-ammoniawater', compin)
...
def Sgenerator(massin, compin, Qgen):
    ...
    entropyout = interpolate('entropy-kjmol-385K-ammoniawater', compin)

    Sgen = symbols('Sgen')
    system = [
    Eq((-1 * massin * entropyin ) + (massout*entropyout) + (Qgen/385) - Sgen, 0)
...
def Qflash(massin, massvapor, massliquid, compin, vaporammonia, liquidammonia):
    ...
    enthalpyoutvapor = interpolate('enthalpy-kjmol-385K-ammoniawater', vaporammonia)

    enthalpyoutliquid = interpolate('enthalpy-kjmol-385K-ammoniawater', liquidammonia)
...
def Sflash(massin, massvapor, massliquid, compin, vaporammonia, liquidammonia, Qflash, T):
    ...
    entropyin = interpolate('entropy-kjmol-385K-ammoniawater', compin)

    entropyoutvapor = interpolate('entropy-kjmol-385K-ammoniawater', vaporammonia)

    entropyoutliquid = interpolate('entropy-kjmol-385K-ammoniawater', liquidammonia)
...
def Qevaporator(m2, m3, m5, ya3, ya2, xa5):
    ...
    enthalpym2 = interpolate('enthalpy-kjmol-385K-ammoniawater', ya2)
    #
...
def Sevaporator(m2, m3, m5, ya3, ya2, xa5, T, Qevap):
    ...
    entropym2 = interpolate('entropy-kjmol-385K-ammoniawater', ya2)
...
def Qabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6):
    ...
    enthalpym4 = interpolate('enthalpy-kjmol-385K-ammoniawater', xa4)
...
def Sabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6, Qabs, T):
    ...
    entropym4 = interpolate('entropy-kjmol-385K-ammoniawater', xa4)
...
def run(ammoniacompin):
    ...
    massvaporoutflash, massliquidoutflash, liquidammoniacomp, vaporammoniacomp = leverrule(10, 385, ammoniacompin)
    ...
    Sgen = Sgenerator(m6new, xa6, 385)

    #print("Qgen: " + str(Qgen) + "\nSgen: " + str(Sgen))

    COPreal = (Qevap/Qgen)
    #Sall = Sgen + Sevap + Sabs
    COPrev = reversibleCOP(266, 325, 385)

    #print(COPrev)
    #Sall = - Qevap/266 - Qabs/325 + Qgen/385
