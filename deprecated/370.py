...
def Qgenerator(massin, compin):
    ...
    enthalpyout = interpolate('enthalpy-kjmol-370K-ammoniawater', compin)
...
def Sgenerator(massin, compin, Qgen):
    ...
    entropyout = interpolate('entropy-kjmol-370K-ammoniawater', compin)

    Sgen = symbols('Sgen')
    system = [
    Eq((-1 * massin * entropyin ) + (massout*entropyout) + (Qgen/370) - Sgen, 0)
...
def Qflash(massin, massvapor, massliquid, compin, vaporammonia, liquidammonia):
    ...
    enthalpyin = interpolate('enthalpy-kjmol-370K-ammoniawater', compin)

    enthalpyoutvapor = interpolate('enthalpy-kjmol-370K-ammoniawater', vaporammonia)

    enthalpyoutliquid = interpolate('enthalpy-kjmol-370K-ammoniawater', liquidammonia)

    Qflash = symbols('Qflash')
    system = [
    Eq(( massin* enthalpyin ) + (-1* massvapor*enthalpyoutvapor) + (-1* massliquid*enthalpyoutliquid) + Qflash, 0)
...
# This calculates the S of the flash
# compin is the ammonia flash
def Sflash(massin, massvapor, massliquid, compin, vaporammonia, liquidammonia, Qflash, T):

    entropyin = interpolate('entropy-kjmol-370K-ammoniawater', compin)

    entropyoutvapor = interpolate('entropy-kjmol-370K-ammoniawater', vaporammonia)

    entropyoutliquid = interpolate('entropy-kjmol-370K-ammoniawater', liquidammonia)

    Sflash = symbols('Sflash')
    system = [
    Eq(( massin* entropyin ) + (-1* massvapor*entropyoutvapor) + (-1* massliquid*entropyoutliquid) + (Sflash/T) - Qflash, 0)
...
def Qevaporator(m2, m3, m5, ya3, ya2, xa5):
    ...
    enthalpym2 = interpolate('enthalpy-kjmol-370K-ammoniawater', ya2)
    #
...
def Sevaporator(m2, m3, m5, ya3, ya2, xa5, T, Qevap):
    ...
    entropym2 = interpolate('entropy-kjmol-370K-ammoniawater', ya2)
...
def Qabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6):
    ...
    enthalpym4 = interpolate('enthalpy-kjmol-370K-ammoniawater', xa4)
...
def Sabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6, Qabs, T):
    ...
    entropym4 = interpolate('entropy-kjmol-370K-ammoniawater', xa4)
...
def run(ammoniacompin):
    ...
    massvaporoutflash, massliquidoutflash, liquidammoniacomp, vaporammoniacomp = leverrule(10, 370, ammoniacompin)
    ...
    Sgen = Sgenerator(m6new, xa6, 370)

    #print("Qgen: " + str(Qgen) + "\nSgen: " + str(Sgen))

    COPreal = (Qevap/Qgen)
    #Sall = Sgen + Sevap + Sabs
    COPrev = reversibleCOP(266, 325, 370)

    #print(COPrev)
    #Sall = - Qevap/266 - Qabs/325 + Qgen/370
