...
def Qgenerator(massin, compin):
	...
	enthalpyout = interpolate('enthalpy-kjmol-365K-ammoniawater', compin)
...
def Sgenerator(massin, compin, Qgen):
	...
	entropyout = interpolate('entropy-kjmol-365K-ammoniawater', compin)

	Sgen = symbols('Sgen')
	system = [
	Eq((-1 * massin * entropyin ) + (massout*entropyout) + (Qgen/365) - Sgen, 0)
...
def Qevaporator(m2, m3, m5, ya3, ya2, xa5):
	...
	enthalpym2 = interpolate('enthalpy-kjmol-365K-ammoniawater', ya2)
    #
...
def Sevaporator(m2, m3, m5, ya3, ya2, xa5, T, Qevap):
	...
	entropym2 = interpolate('entropy-kjmol-365K-ammoniawater', ya2)
...
def Qabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6):
	...
	enthalpym4 = interpolate('enthalpy-kjmol-365K-ammoniawater', xa4)
...
def Sabsorber(m3, m4, m5, m6, ya3, xa4, xa5, xa6, Qabs, T):
	...
	entropym4 = interpolate('entropy-kjmol-365K-ammoniawater', xa4)
...
def run(ammoniacompin):
	...
	massvaporoutflash, massliquidoutflash, liquidammoniacomp, vaporammoniacomp = leverrule(massintoflash, 365, compammoniagen)
	...
	COPrev = reversibleCOP(266, 325, 365)
